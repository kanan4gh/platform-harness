...
スタックPR(ベースが別のフィーチャーブランチのPR)はマージ順事故の温床のため、原則として避ける:

- **可能な限り、ベースのマージを待って通常PR(base: main)にする**。複数の変更を並行させる場合は、PR間で変更ファイルを重複させず任意順でマージ可能にする
- やむを得ずスタックPRを使う場合は、**必ずベース側から順にマージする**。PR本文の依存注記だけでは機構的に防げない(実例: 先にスタック側をマージした結果、変更がベースブランチ止まりでmainに届かない事故が発生)
- **マージ後は「デフォルトブランチに期待する変化が実在するか」を検証する**(例: 変更したはずの設定値をmainで確認、Closesが発火してIssueが閉じたか確認)
...
ハーネス中立の文書を書くとき、ツール名だけでなく**そのツールの機能の呼称**(例: Claude Codeの「プロジェクトメモリ」)も監査対象にする。著者には自分の語彙の染み込みが見えないため、著者と別の実行主体(可能なら別モデル)によるレビューが有効(実例: 別モデルのレビュアが文書タイトルの固有語彙を検出した)。

### 非対話実行の罠 — 検証はログと状態で判定する

headless実行(`claude -p` / `codex exec`)はセッション系機構(フック・trust承認)の挙動が対話時と異なる。検証設計時は対話/非対話の差を最初に確認し、ハーネス機構の影響を受けない隔離環境で検証する。エージェントの完走報告を成功の根拠にせず、**ログと状態(設定ファイル・状態ファイル)で判定**する(実例: 完走したように見えたセッションで、フックがtrust未承認のため一度も動いていなかった)。
